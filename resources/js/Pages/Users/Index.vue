...
<template>
    ...
    <div class="max-w-7xl mx-auto px-6 py-8">
        ...
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-3xl font-semibold text-gray-200 ml-12">Gestión de Usuarios</h2>
        ...
        <div class="overflow-x-auto bg-neutral-800 shadow-lg rounded-lg  m-7">
            ...
            <table class="min-w-full table-auto">
                <thead class="bg-black text-gray-300">
                    <tr>
                        <th class="px-6 py-3 text-left text-sm font-semibold uppercase tracking-wider border-b border-gray-700">
                            ID
                        </th>
                        <th class="px-6 py-3 text-center text-sm font-semibold uppercase tracking-wider border-b border-gray-700">
                            Nombre
                        </th>
                        <th class="px-6 py-3 text-left text-sm font-semibold uppercase tracking-wider border-b border-gray-700">
                            Correo
                ...
                <tbody>
                    <tr v-for="user in users.data" :key="user.id"
                        ...
                        <td class="px-6 py-4 text-sm font-medium text-gray-300">{{ user.id }}</td>
                        <td class="px-6 py-4 text-sm">{{ user.name }}</td>
                        <td class="px-6 py-4 text-sm text-justify text-gray-300">{{ user.email }}</td>
                        <td class="px-6 py-4 text-sm">
                            <span class="text-xs text-gray-400">{{ user.created_at }}</span>
                        </td>
                        <td class="px-6 py-4 text-sm">
                            <span class="text-xs text-gray-400">{{ user.updated_at }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <!-- Edit Button -->
