...
defineProps({
    ...
    let categoriasUrl = computed(() => {
        let url = new URL(route("categorias.index"));
    ...
    watch(
        () => categoriasUrl.value,
        (updatedCategoriasUrl) => {
            router.visit(updatedCategoriasUrl,{
                preserveScroll: true,
                preserveState: true,
                replace: true,
            });
    ...
    const deleteCategoria = (categoriaId) => {
        if(confirm('¿Estás seguro de eliminar esta categoría?')) {
            deleteForm.delete(route('categorias.destroy', categoriaId));
        }

    }
    // console.log(usePage().props.categorias);
...
<template>
    ...
    <div class="max-w-7xl mx-auto px-6 py-8">
        ...
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-3xl font-semibold text-gray-200 ml-12">Gestión de Categorías</h2>
            <Link as="button" :href="route('categorias.create')"
        ...
        <div class="overflow-x-auto bg-neutral-800 shadow-lg rounded-lg  m-7">
            ...
            <table class="min-w-full table-auto">
                ...
                <tbody>
                    ...
                    <tr v-for="categoria in categorias.data" :key="categoria.id"
                        ...
                        <td class="px-6 py-4 text-sm font-medium text-gray-300">{{ categoria.id }}</td>
                        <td class="px-6 py-4 text-sm">{{ categoria.categoria }}</td>
                        <td class="px-6 py-4 text-sm text-justify text-gray-300">{{ categoria.descripcion }}</td>
                        <td class="px-6 py-4 text-sm">
                            {{ categoria.created_by.name }} <br />
                            <span class="text-xs text-gray-400">{{ categoria.created_at }}</span>
                        </td>
                        <td class="px-6 py-4 text-sm">
                            {{ categoria.updated_by.name }} <br />
                            <span class="text-xs text-gray-400">{{ categoria.updated_at }}</span>
                        </td>
                        <td class="px-6 py-4">
                            ...
                            <Link
                                ...
                                :href="route('categorias.destroy', categoria.id)"
            ...
            <Pagination :data="categorias" :updatedPageNumber="updatedPageNumber" />
        </div>
    </div>
</template>
